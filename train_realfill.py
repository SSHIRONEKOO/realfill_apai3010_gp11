...
class RealFillDataset(Dataset):
    ...
    def __init__(
        ...
        # Use transforms_v2 for resize/crop only; convert to tensor with pil_to_tensor
        self.resize = transforms_v2.RandomResize(size, int(1.125 * size))
        self.crop = transforms_v2.RandomCrop(size)
    ...
    def __getitem__(self, index):
        ...
        # Apply resize and crop (PIL operations)
        image = self.resize(image)
        image = self.crop(image)
        weighting = self.resize(weighting)
        weighting = self.crop(weighting)

        # Convert to tensor and normalize to [-1, 1]
        image = pil_to_tensor(image).float() / 255.0  # [C,H,W], float32
        weighting = pil_to_tensor(weighting).float() / 255.0
        image = image * 2.0 - 1.0
        weighting = weighting * 2.0 - 1.0
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet, text_encoder):
                ...
                weightings = batch["weightings"].to(accelerator.device, dtype=weight_dtype)
